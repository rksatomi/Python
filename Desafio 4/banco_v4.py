...
# selecionar o caminho do diretório
...
class Conta:
    ...
    def depositar(self, valor):
        if valor > 0:  # condiçao de depósito maior que 0
            # adiciona valor de depósito ao saldo, desde de que seja aceita a condição anterior
            self._saldo += valor
            print("Valor depositado R$ %.2f com sucesso!" % valor + "\n")
        else:
            print(
                "Erro de Operação! O valor informado é inválido."
            )  # resultado da condiçao de depósito inferior a  0
            return False
        return True
...
class Saque(Transacao):
    ...
    def registrar(self, conta):
        # verificação de requisitos para realização de saque
        # (valor > saldo e 0 / ser cliente cadastrado)
...
def log_trasacao(func):
    def envelope(
        *args, **kwargs
    ):  # indicação de argumentos variaveis sem necessidade de reajustar de acordo com o problema
        data_hora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            # abertura de arquivo
            with open(ROOT_PATH / "log.txt", "a", encoding="utf-8") as arquivo:
                resultado = func(*args, **kwargs)
                # definição de retorno, neste probema só necessitamos do print
                arquivo.write(
                    f"""[{data_hora}]\tFunção '{func.__name__.upper()}'\t
                    Executado com argumentos{args} e {kwargs}\t Retornou {resultado}\n"""
                )
            return resultado
        except FileNotFoundError as exc:
            print(f"Arquivo não existente, criação de arquivo - {exc}")
        except IOError as exc:
            print(f"Erro ao abrir o arquivo: {exc}")
        except Exception as exc:
            print(f"Algum erro ocorreu na abertura de arquivo:{exc}")
        arquivo.close()
        print("Criação de Log")
    return envelope
...
def criar_clientes(clientes):
    ...
    cliente = PessoaFisica(
        nome=nome, dataNascimento=data_nascimento, cpf=cpf, endereco=endereco
    )
